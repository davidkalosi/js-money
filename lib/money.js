...
function Money(amount, currency, options) {
    ...
    this.amount = options && options.skipDecimals ? amount : amount * multipliers[currency.decimal_digits];    
...
Money.prototype.add = function(other) {
    ...
    return new Money(self.amount + other.amount, self.currency, {skipDecimals: true});
};
...
Money.prototype.multiply = function(multiplier) {
    ...
    return new Money(amount, this.currency, {skipDecimals: true});
};
...
Money.prototype.allocate = function(ratios) {
    ...
    ratios.forEach(function(ratio) {
        ...
        results.push(new Money(share, self.currency, {skipDecimals: true}));
        remainder -= share;
    });

    for (var i = 0; remainder > 0; i++) {
        results[i] = new Money(results[i].amount + 1, results[i].currency, {skipDecimals: true});
        remainder--;
    }
